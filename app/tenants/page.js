...
function TenantsContent() {
  ...
  async function handleCashPayment() {
    ...
    try {
      ...
      const res = await fetch('/api/payments', {
        ...
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({
          paymentId: getUnpaidPayment()?._id,
  ...
  async function handleOnlinePayment() {
    ...
    try {
      ...
      const res = await fetch('/api/payment/create-order', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ tenantId: selected._id, amount: selected.rentAmount, month: thisMonth, year: thisYear }),
      })
  ...
  function getUnpaidPayment() {
    ...
    const tp = tenants.find(t => t._id === selected._id)
    return tp?.payments?.find(p => p.status !== 'paid')
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0f172a', color: '#f1f5f9' }}>
      ...
      <div style={{ display: 'flex', height: 'calc(100vh - 60px)' }}>
        ...
        <div style={{ width: '280px', minWidth: '280px', borderRight: '1px solid #334155', overflowY: 'auto', background: '#0f172a' }}>
          ...
          {loading ? (
            ...
            const paid = t.payments?.some(p => p.month === thisMonth && String(p.year) === String(thisYear) && p.status === 'paid')
            return (
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '24px 28px' }}>
          {!selectedFresh ? (
            ...
            <>
              {msg && (
                <div style={{ background: msg.startsWith('✅') ? '#14532d' : '#7f1d1d', border: `1px solid ${msg.startsWith('✅') ? '#22c55e' : '#ef4444'}`, borderRadius: '8px', padding: '10px 16px', marginBottom: '20px', color: msg.startsWith('✅') ? '#86efac' : '#fca5a5', fontSize: '14px' }}>{msg}</div>
              )}

              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                <div>
                  <h2 style={{ margin: 0, fontSize: '20px', fontWeight: 700 }}>{selectedFresh.name}</h2>
                  <p style={{ margin: '4px 0 0', color: '#64748b', fontSize: '13px' }}>Room {selectedFresh.roomNumber} · ₹{selectedFresh.rentAmount}/month</p>
                </div>
              ...
              {!editMode ? (
                <>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', marginBottom: '20px' }}>
                    {[
                      ...
                      { label: 'Join Date', value: selectedFresh.startDate ? new Date(selectedFresh.startDate).toLocaleDateString('en-IN') : selectedFresh.joinDate ? new Date(selectedFresh.joinDate).toLocaleDateString('en-IN') : '—', color: '#94a3b8' },
                    ].map((item, i) => (
                  ...
                  <div style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: '12px', padding: '18px 20px', marginBottom: '20px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      {selectedPaid ? (
                        ...
                        <button onClick={() => setShowPayModal(true)} disabled={payLoading} style={{ background: 'linear-gradient(135deg, #3b82f6, #2563eb)', border: 'none', color: 'white', padding: '10px 20px', borderRadius: '8px', cursor: 'pointer', fontWeight: 600, fontSize: '14px' }}>
                ...
                <form onSubmit={handleEdit} style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: '12px', padding: '24px' }}>
                  ...
                  <button type="submit" style={{ marginTop: '20px', padding: '11px 28px', background: 'linear-gradient(135deg, #10b981, #059669)', border: 'none', borderRadius: '8px', color: 'white', fontWeight: 600, cursor: 'pointer' }}>
                    Save Changes
                  </button>
                </form>
              )}
      ...
      {/* Payment Modal */}
      {showPayModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }} onClick={() => setShowPayModal(false)}>
          <div style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: '16px', padding: '32px', width: '360px' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <button onClick={handleCashPayment} disabled={payLoading} style={{ display: 'flex', alignItems: 'center', gap: '16px', background: '#0f172a', border: '2px solid #334155', borderRadius: '12px', padding: '18px 20px', cursor: 'pointer', color: '#f1f5f9', textAlign: 'left' }}>
              ...
              <button onClick={handleOnlinePayment} disabled={payLoading} style={{ display: 'flex', alignItems: 'center', gap: '16px', background: '#0f172a', border: '2px solid #334155', borderRadius: '12px', padding: '18px 20px', cursor: 'pointer', color: '#f1f5f9', textAlign: 'left' }}>
                <span style={{ fontSize: '32px' }}>💳</span>
                <div>
            ...
            <button onClick={() => setShowPayModal(false)} style={{ marginTop: '16px', width: '100%', padding: '10px', background: 'transparent', border: '1px solid #334155', borderRadius: '8px', color: '#64748b', cursor: 'pointer' }}>Cancel</button>
          </div>
        </div>
      )}
    </div>
